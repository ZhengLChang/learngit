...
long tv_diff(struct timespec* chk_start_time)
{
	struct timespec tv;
	time_t usec;
    clock_gettime(CLOCK_MONOTONIC, &tv);
    usec = (tv.tv_sec - chk_start_time->tv_sec) * 1000000;
    usec += (tv.tv_nsec - chk_start_time->tv_nsec) / 1000;
	return usec;
...
int arpCheck()
{
  ...
  clock_gettime(CLOCK_MONOTONIC, &chk_start_time);
  do
    {
      do
    	{
      	  if ( i++ > 4 || tv_diff(&chk_start_time) > 50000000 ) return 0; /*  5 probes  */
#ifdef OLD_LINUX_VERSION
      ...
      do
    	{
		  /*Avoid dead loop when lan has lots of packets*/
		  if (tv_diff(&chk_start_time) > 50000000)
				return 0;
