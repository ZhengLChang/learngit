...
#define NET_MSG_L2TP_CONNECTING      0x000020
#define NET_MSG_L2TP_DISCONNECTED    0x000021
...
int sendXl2tpMonitorMsg(int code)
{
...
struct call *lac_call (int tid, struct lac *lac, struct lns *lns)
{
    ...
    while (t)
    {
	    ...
	    sendXl2tpMonitorMsg(NET_MSG_L2TP_CONNECTING);
...
void lac_disconnect (int tid)
{
    ...
    while (t)
    {
        if (t->ourtid == tid)
        {
            ...
            call_close (t->self);
	    sendXl2tpMonitorMsg(NET_MSG_L2TP_DISCONNECTED);
            return;
        }
        t = t->next;
    };
