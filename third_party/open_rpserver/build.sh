...
#else
	make -C src AR=${IFX_AR} STRIP=${IFX_STRIP} AS=${IFX_AS} LD=${IFX_LD} NM=${IFX_NM} CC=${IFX_CC} BUILDCC=${IFX_HOSTCC} GCC=${IFX_CC} CXX=${IFX_CXX} CPP=${IFX_CPP} RANLIB=${IFX_RANLIB} IFX_CFLAGS="${IFX_CFLAGS}" IFX_LDFLAGS="${IFX_LDFLAGS}" install DESTDIR=${BUILD_ROOTFS_DIR}
#fi
ifx_error_check $? 
